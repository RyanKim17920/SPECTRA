...
#
# RUN NAME UNIQUENESS IS LOAD-BEARING. SPECTRA_RUN_NAME becomes PretrainedModel.name, the
...
if [ "$MODE" != "--thunder-only" ]; then
  jid=$(SPECTRA_RUN="$RUN" SPECTRA_STEP="$STEP" sbatch --parsable --account=max --qos=high \
        --job-name="hest-${TAG}" \
        --export=ALL,SPECTRA_RUN="$RUN",SPECTRA_STEP="$STEP" \
...
if [ "$MODE" != "--hest-only" ]; then
  ...
  # from SPECTRA_BACKBONE (CLS+mean for Virchow2/Midnight, CLS elsewhere) rather than us
  # hardcoding one protocol across three backbones.
  for ds in $CLS_DS; do
    jid=$(SPECTRA_BACKBONE="$BACKBONE" sbatch --parsable --account=max --qos=high \
          --job-name="thd-${TAG}-${ds}" \
  ...
  for ds in $SEG_DS; do
    ...
    # pannuke are not those, so no SPECTRA_EPOCHS override here.
    jid=$(SPECTRA_BACKBONE="$BACKBONE" sbatch --parsable --account=max --qos=high \
          --job-name="thd-${TAG}-${ds}" \
          --export=ALL,SPECTRA_BACKBONE="$BACKBONE" \
