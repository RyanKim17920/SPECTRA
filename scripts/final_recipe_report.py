...
The criterion this script grades against (deliberately NOT softened):
    ...
    The 100 cap is a REPORTING convention ("exceeding Waiv counts as 100") and is applied
    only AFTER the resolution test, to the displayed/averaged value.  The >=70 test runs
    on the UNCAPPED interval -- capping first and then subtracting the CI censors the
    point estimate but not the error bar, which manufactures NOT RESOLVED verdicts.
...
HEST_POOLING = {a: _c5.hest_pooling(a) for a in ("phikon", "midnight", "virchow2")}
...
def grade(pct_uncapped: float | None, ci: float | None, n: int | None = None) -> dict:
    ...
    One place owns the cap-vs-test ORDER so RI, HEST and THUNDER cannot drift apart:
    test on the uncapped interval, then cap only what is displayed/averaged.
    """
    ...
    return {
        "pct": capped,                       # what feeds the averages (user's convention)
        "pct_capped": capped,
...
def fmt_cell(c: dict) -> str:
    ...
    unc = c.get("pct_uncapped")
    unc_s = f"[{unc:5.1f}]" if unc is not None else ""
    return f"{c['pct']:6.1f}{star}{unc_s} {ci_s} n={c.get('n', 0)}  {c['status']}"
...
def build_report(hest_assume_step: int | None = None) -> dict:
    ...
    report: dict = {
        "criterion": {
            ...
            "pct_capped_at": 100,
            "cap_is_reporting_only": ("the 100 cap is applied AFTER the >=70 resolution "
                                      "test and only to the displayed/averaged value; "
                                      "the test runs on the uncapped interval"),
    ...
    for a in ARMS:
        ...
        pcts = [cap100(p) for p in pcts_unc]
        mean_pct = sum(pcts) / n                 # capped: the reported/averaged number
        mean_pct_unc = sum(pcts_unc) / n         # uncapped: what the 70 bar is tested on
        ...
        if n >= 2:
            # F14 fix (2026-08-26): compute the empirical SD from the UNCAPPED pcts.
            # It used to be taken over the CAPPED pcts about the capped mean -- exactly
            # the censored-SD problem this block's own comment warns about: two seeds
            # both above Waiv both become exactly 100.0, SD collapses to 0, and the cell
            # claims infinite precision.  The uncapped spread is the real measurement.
            emp_sd = math.sqrt(sum((q - mean_pct_unc) ** 2 for q in pcts_unc) / (n - 1))
        ...
        cell.update({
            ...
            "per_seed_pct": pcts, "per_seed_pct_uncapped": pcts_unc,
            "pct_capped_mean_of_capped_seeds": mean_pct,
            "ci_source": ci_src,
        })
        # The averages use the capped-per-seed mean (unchanged convention); the cap is
        # applied per seed, exactly as before, and never before the resolution test.
        cell["pct"] = mean_pct
        cell["pct_capped"] = mean_pct
    ...
    for a in ARMS:
        ...
        if len(sup_rows) > n:
            ...
            s_cap = [cap100(q) for q in s_unc]
            s_ci = 2.0 * sd / math.sqrt(sn)
            scell = grade(sum(s_unc) / sn, s_ci, sn)
            scell.update({
                "n": sn,
                "raw_mean": sum(sv) / sn, "base": HEST_BASE[a], "waiv": HEST_WAIV[a],
                "per_seed_pct": s_cap, "per_seed_pct_uncapped": s_unc,
                "pooling": HEST_POOLING[a],
            ...
            if scell["status"] != "UNDERPOWERED":
                scell["pct"] = sum(s_cap) / sn
                scell["pct_capped"] = scell["pct"]
    ...
    for a in ARMS:
        ...
        for task in CLS_TASKS:
            ...
            ci = (2.0 * seed_sd / abs(waiv_gain) * 100.0 / math.sqrt(n)
                  if seed_sd is not None else None)
            entry["resolvability_floor_offset_2se"] = floor
            entry["ci_source"] = (
                (f"2 * seed_SD_12ds({seed_sd:.6f}) / |waiv_gain|({abs(waiv_gain):.4f})"
                 f" * 100 / sqrt({n})") if seed_sd is not None
            ...
            if entry["status"] == "UNDERPOWERED":
                ...
                continue
            entry.update({
                "pct": mean_pct, "pct_capped": mean_pct, "n": n,
        ...
        if not graded:
            ...
            cell.update({"pct": (mean_pct if cell["status"] != "UNDERPOWERED" else None),
                         "pct_capped": (mean_pct if cell["status"] != "UNDERPOWERED" else None),
                         "n": agg_n,
                         "tasks": tasks_out,
                         "tasks_graded": graded,
    ...
    if ungraded_cells or unresolved:
        ...
        if unresolved:
            bits.append("error bar straddles the 70 bar for " +
                        ", ".join(f"{a}/{b} ({c['pct_uncapped']:.1f}+/-{c['ci']:.1f} uncapped)"
                                  for a, b, c in unresolved))
        reason = "; ".join(bits)
    elif failed:
        verdict = "FAIL"
        reason = "below the 70 bar: " + ", ".join(
            f"{a}/{b} = {c['pct_uncapped']:.1f}+/-{c['ci']:.1f} uncapped" for a, b, c in failed)
        ...
        reason = (f"worst cell {worst[0]}/{worst[1]} = {worst[2]['pct']:.1f} "
                  f"(uncapped {worst[2]['pct_uncapped']:.1f}, CI lower bound "
                  f"{worst[2]['lower_uncapped']:.1f}) >= {PASS_BAR}; "
...
def print_report(rep: dict) -> None:
    ...
    print("                The >=70 test runs on the UNCAPPED interval; the 100 cap is applied")
    print("                only afterwards, to the displayed/averaged value (marked *).")
    print("                Scorecard cells read:  capped* [uncapped] +/-CI n=  status")
    print()
    ...
    for a in ARMS:
        ...
        for t in CLS_TASKS:
            ...
            if e.get("pct") is not None:
                ...
                print(f"{head} {e['pct']:6.1f}{star}[{e['pct_uncapped']:6.1f}] "
    ...
    if w:
        print(f"  WORST cell: {w['backbone']}/{w['benchmark']} = {w['pct']:.1f} "
              f"(uncapped {w['pct_uncapped']:.1f} +/-{w['ci']:.1f}, "
              f"lower {w['lower_uncapped']:.1f})  [{w['status']}]   (bar: >= {PASS_BAR:.0f})")
