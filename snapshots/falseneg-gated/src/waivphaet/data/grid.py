...
class GridBatchSampler(Sampler[GridBatch]):
    ...
    def __init__(
        ...
        *,
        ...
        #: Tissue-concentrated sampling. With core_labels and cores_per_batch > 0 each step
        #: draws its tiles from only that many randomly chosen tissue cores instead of the
        #: whole slide, RAISING the same-tissue fraction of the negative set from ~1/46
        #: (2.4%) to ~1/cores_per_batch. Direct inverse of --mask-same-core, which removed
        #: those negatives and cost HEST at every dose.
        self.core_labels = None if core_labels is None else np.asarray(core_labels)
        self.cores_per_batch = int(cores_per_batch)
        if self.core_labels is not None and self.cores_per_batch > 0:
            _lab = self.core_labels[self.tiles]
            self._core_tiles = [np.where(_lab == c)[0] for c in np.unique(_lab)]
        else:
            self._core_tiles = None
        self.seed = seed
        self.epoch = 0
    ...
    def _draw(self, rng: np.random.Generator, n_available: int) -> GridBatch:
        ...
        if self._core_tiles is not None:
            k = min(self.cores_per_batch, len(self._core_tiles))
            pick = rng.choice(len(self._core_tiles), size=k, replace=False)
            pool = np.concatenate([self._core_tiles[i] for i in pick])
            if pool.size < self.n_tiles:  # k cores too small -- fall back to full slide
                tile_idx = rng.choice(self.tiles, size=self.n_tiles, replace=False)
            else:
                tile_idx = self.tiles[rng.choice(pool, size=self.n_tiles, replace=False)]
        else:
            tile_idx = rng.choice(self.tiles, size=self.n_tiles, replace=False)
        return GridBatch(
