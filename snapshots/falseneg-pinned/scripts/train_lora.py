...
def parse_args():
    ...
    ap.add_argument("--mask-same-core", action="store_true",
                    help="Mask same-organ-core tiles from each anchor's negative set in the "
                         "GRID InfoNCE loss. Requires runs/.plism_core_labels.npy (produced by "
                         "scripts/derive_core_map.py). The positive is NEVER masked. Default OFF. "
                         "Only effective with --grid.")
    ap.add_argument("--same-core-logit-bias-cls", type=float, default=None,
                    help="Per-head override of --same-core-logit-bias for the CLS head. "
                         "HEST(phikon) reads CLS only, so beta>1 here targets HEST.")
    ap.add_argument("--same-core-logit-bias-mean", type=float, default=None,
                    help="Per-head override for the MEAN head. RI reads clsmean, so "
                         "beta<1 here preserves robustness while CLS chases HEST.")
    ap.add_argument("--center-embeddings", action="store_true",
                    help="Subtract the batch mean before L2-normalising in the grid loss. "
                         "~50%% of the fine-tuning update is a shared shift that a centered "
                         "linear readout (HEST: scaler->PCA->ridge) cannot see; centering "
                         "makes that direction give zero loss reduction.")
    ap.add_argument("--same-core-logit-bias", type=float, default=None,
                    help="log(beta) added to same-tissue negative logits when "
                         "--mask-same-core is set. Omit for the hard mask (beta=0); "
                         "0.0 = baseline; >0 upweights same-tissue hard negatives, which "
                         "carry the HEST-relevant gradient. Requires --core-labels-path.")
    ap.add_argument("--cores-per-batch", type=int, default=0,
                    help="Draw each step's tiles from only this many tissue cores, "
                         "RAISING the same-tissue fraction of the negative set from ~1/46 "
                         "to ~1/N. Inverse of --mask-same-core. 0 = OFF (uniform).")
    ap.add_argument("--mask-sim-thresh", type=float, default=0.0,
                    help="Mask negatives whose FROZEN base-model cosine to the anchor "
                         "exceeds this threshold. Targets near-duplicates only, unlike "
                         "--mask-same-core which masks a whole organ core. 0 = OFF.")
    ap.add_argument("--sim-emb-path", type=str,
                    default="/admin/home/ryan.kim/waiv/runs/.plism_ref_emb.npy",
                    help="Frozen base-model embeddings used by --mask-sim-thresh.")
    ap.add_argument("--core-labels-path", type=str,
                    default="/admin/home/ryan.kim/waiv/runs/.plism_core_labels.npy",
                    help="Path to per-tile core label array (int32, len 16278). "
                         "Used when --mask-same-core is set.")
...
def main() -> int:
    ...
    if args.grid:
        ...
        _core_lab = None
        if args.cores_per_batch > 0:
            _core_lab = np.load(args.core_labels_path).astype(np.int32)
            print(
                f"[train] TISSUE CONCENTRATION: {args.cores_per_batch} cores/batch from "
                f"{len(np.unique(_core_lab))} tissues -- same-tissue negative fraction "
                f"~{1.0/args.cores_per_batch:.1%} (uniform baseline "
                f"~{1.0/len(np.unique(_core_lab)):.1%})", flush=True
            )
        train_loader = build_grid_loader(
            ...
            core_labels=_core_lab, cores_per_batch=args.cores_per_batch,
        )
    ...
    cfg = TrainConfig(
        ...
        cores_per_batch=args.cores_per_batch if args.grid else 0,
        same_core_logit_bias=args.same_core_logit_bias,
        center_embeddings=args.center_embeddings,
        same_core_logit_bias_cls=args.same_core_logit_bias_cls,
        same_core_logit_bias_mean=args.same_core_logit_bias_mean,
        mask_same_core=args.mask_same_core if args.grid else False,
        core_labels_path=args.core_labels_path,
        mask_sim_thresh=args.mask_sim_thresh if args.grid else 0.0,
        sim_emb_path=args.sim_emb_path,
